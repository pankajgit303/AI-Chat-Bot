...
export function CodeBlock({ inline, className, children, ...props }: CodeBlockProps) {
  ...
  const codeContent = (
    <div className={`relative group border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-900 my-4 shadow-sm ${isFullscreen ? 'h-full flex flex-col my-0' : ''}`}>
      ...
      <pre className={`overflow-x-auto m-0 p-0 bg-transparent ${isFullscreen ? 'flex-1 overflow-y-auto p-6' : 'pt-10 pb-4 px-4'}`}>
  ...
  if (isFullscreen) {
    return (
      <>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 md:p-8" onClick={toggleFullscreen}>
          <div 
        ...
        <div className="relative group rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm p-4 text-center text-slate-500 dark:text-slate-400 italic cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors" onClick={toggleFullscreen}>
