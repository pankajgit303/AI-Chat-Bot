...
import { MessageSquare, Plus, Trash2, Settings, Info, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
    ...
    <>
      ...
      >
        <div className="p-6">
          ...
          <button
            onClick={onNewChat}
            className="w-full bg-slate-800 hover:bg-slate-700 text-white rounded-xl py-3 px-4 flex items-center gap-3 transition-all mb-6 border border-slate-700/50"
          >
            <Plus size={20} />
        ...
        <div className="mt-auto p-6 space-y-4 border-t border-slate-800">
          <div className="bg-slate-800/50 p-4 rounded-xl cursor-pointer hover:bg-slate-800/70 transition-colors" onClick={onOpenAbout}>
            <p className="text-[10px] uppercase font-bold text-slate-500 mb-2">Portfolio Project</p>
            <h4 className="text-xs font-semibold text-white mb-1">Developed by: Pankaj Singha</h4>
            <p className="text-[10px] text-slate-400">BCA – 2nd Year • React + Gemini</p>
          </div>
          <div className="flex gap-2">
            <button onClick={onOpenSettings} className="flex-1 py-2 px-3 bg-slate-800 rounded-lg text-xs hover:bg-slate-700 flex items-center justify-center gap-2 text-slate-300">
              <Settings size={16} />
              Settings
            </button>
            <button onClick={onOpenAbout} className="py-2 px-3 bg-slate-800 rounded-lg hover:bg-slate-700 text-slate-300">
              <Info size={16} />
            </button>
          </div>
        </div>
