...
export function ChatMessage({ message, isLastModelMessage, onRegenerate, isRegenerating }: ChatMessageProps) {
  ...
  return (
    <div className={`flex w-full py-6 px-4 md:px-8 ${isUser ? 'bg-transparent' : 'bg-slate-50 dark:bg-slate-900/50'}`}>
      <div className="max-w-4xl mx-auto flex gap-4 md:gap-6 w-full group">
        ...
        <div className="flex-1 min-w-0 flex flex-col pt-1">
            ...
            <ReactMarkdown 
              ...
              components={{
                pre({node, children, ...props}: any) {
                  ...
                  return (
                    <CodeBlock inline={false} className={childProps.className} {...childProps}>
                      {childProps.children}
                    </CodeBlock>
                  );
                },
          ...
          {!isUser && (
            <div className={`flex items-center gap-2 mt-4 transition-opacity ${isLastModelMessage ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
              <button 
                onClick={handleCopy}
